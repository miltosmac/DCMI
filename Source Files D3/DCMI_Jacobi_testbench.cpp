...
int main(int argc, char **argv)
{
	...
	txtfile.close();
	//Calculate The Appropriate Coefficient Arrays for D=3
	...
	for (int i=0; i<HEIGHT; i++){
		...
		// Check HW result against SW
			if (!testream_out.empty()){ //Values read from stream and into the HW result variable for checking
				testream_out.read(hw_result[i][j]);
			}
			mytxt << hw_result[i][j]<<"  ";
